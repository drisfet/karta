...
const detectEnvironment = () => {
  ...
  console.log('🚀 Starting Morphic Agent API...');
...
fastify.get('/test', async () => {
  return {
    message: 'Morphic Agent API is running and accessible!',
    timestamp: new Date().toISOString(),
    cors_test: 'success'
  };
});
...
const start = async () => {
  try {
    ...
    console.log('✅ Morphic Agent API started successfully!');
    ...
    console.log('─'.repeat(60));
    console.log('🎉 Morphic Agent API is ready to accept requests!');
  } catch (err) {
    console.error('❌ Failed to start Morphic Agent API:', err);
