...
import { PanelConfig, PanelRegistry, PanelType } from './panel-registry';
      ...
      {minimizedPanels.length > 0 && (
        <div className="absolute top-0 left-20 right-0 h-16 bg-black/10 backdrop-blur-md p-2 flex items-center gap-2 overflow-x-auto z-50">
          {minimizedPanels.map(panel => (
            <div
              key={panel.id}
              className="bg-neutral-800/80 backdrop-blur-md rounded-lg px-4 py-2 flex items-center gap-2 cursor-pointer hover:bg-neutral-700/80 transition-colors"
              onClick={() => handlePanelMinimize(panel.id)}
            >
              <span className="text-sm text-white truncate max-w-32">{panel.title}</span>
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  handlePanelClose(panel.id);
                }}
